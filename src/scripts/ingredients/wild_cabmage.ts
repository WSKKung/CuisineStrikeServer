...
function targetFilter(card: Card, ownerCard: Card) {
	return !Card.isSame(card, ownerCard) && Card.hasType(card, CardType.INGREDIENT) && Card.hasClass(card, CardClass.VEGETABLE);
}

const WILD_CABMAGE_EFFECT: CardEffect = {
	type: "activate",
	condition(context) {
...
export default WILD_CABMAGE_EFFECT;