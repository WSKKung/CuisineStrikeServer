...
function targetFilter(card: Card, ownerCard: Card) {
	return !Card.isSame(card, ownerCard) && Card.hasType(card, CardType.INGREDIENT) && Card.hasClass(card, CardClass.MEAT);
}

const DREAMY_SHLEEP_EFFECT: CardEffect = {
	type: "activate",
	condition(context) {
...
export default DREAMY_SHLEEP_EFFECT;