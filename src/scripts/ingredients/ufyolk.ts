...
function targetFilter(card: Card, ownerCard: Card) {
	return !Card.isSame(card, ownerCard) && Card.hasType(card, CardType.INGREDIENT) && Card.hasClass(card, CardClass.EGG);
}

const UFYOLK_EFFECT: CardEffect = {
	type: "activate",
	condition(context) {
